...
export function AdminAppsPage() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const res = await adminService.getAllApps();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-black text-white">All Applications</h1>
        <p className="text-[#94A3B8] mt-1">View all apps created by users across the platform.</p>
      </div>

      <div className="bg-[#0F172A] border border-[#1E293B] rounded-2xl p-4 flex gap-3 items-center">
            ...
            <thead>
              <tr className="border-b border-[#1E293B] bg-[#0B1220]/50">
                {["App Name", "App ID", "Owner", "Created"].map(h => (
                  <th key={h} className="py-4 px-6 text-[10px] font-black text-[#4A5568] uppercase tracking-widest">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-[#1E293B]/30">
              {loading ? (
                ...
                <motion.tr key={app.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="group hover:bg-[#7C3AED]/5 transition-colors">
                  ...
                  <td className="py-4 px-6">
                    <code className="text-xs text-[#7C3AED] font-mono bg-[#7C3AED]/5 px-2 py-1 rounded">{app.appId}</code>
                  </td>
                  <td className="py-4 px-6">
                    <p className="text-sm font-bold text-white">{app.user?.name || "—"}</p>
                    <p className="text-[10px] text-[#4A5568]">{app.user?.email || "—"}</p>
                  </td>
